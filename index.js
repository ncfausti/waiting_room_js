...
function formatTime(floor, now, ceiling) {
  ...
  if (timeToFloor > 0 && timeToCeiling > 0) {
    ...
    const hoursCeiling = minutesCeiling >= 120 ? '2h' :  Math.floor(minutesCeiling / 60);
    
    const minutesFloor = timeToFloor / 1000 / 60;
    const hoursFloor = minutesFloor >= 120 ? '2h' :  Math.floor(minutesFloor / 60);
    ...
    if (minutesFloor >= 60 && hoursCeiling === 1) {
    	return ['1h'+ toFive(minutesFloor % 60) + 'm', 
      	(hoursCeiling+"h" + toFive(minutesCeiling % 60) + 'm')].join('-');
    }
    ...
    if (minutesFloor < 60 && hoursCeiling === 0) {
    	return [toFive(minutesFloor % 60) + 'm', toFive(minutesCeiling)+'m'].join('-');
    }
  }
}
